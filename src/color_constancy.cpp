    ...
    *ml=0;
      ...
      for(int i=0;i<src1.rows;i++)
      {
          ...
          for(int j=0;j<src1.cols;j++)
          {
              ...
              float lc=pow((float)v1.val[0],p);
              float ac=pow((float)v1.val[1],p);
              float bc=pow((float)v1.val[2],p);
              *ma=*ma+ac;
              *mb=*mb+bc;
              *ml=*ml+lc;
...
void color_correction::gray_world::process(Mat src1,float *ml,float *ma,float *mb,int p,int m)
{
     ...
     *ml=0;
       ...
       for(int i=0;i<src1.rows;i++)
       {
           ...
           {
               Vec3b v1=src1.at<cv::Vec3b>(i,j);
               float lc=(int)pow((float)v1.val[0],p);
               float ac=(int)pow((float)v1.val[1],p);
               float bc=(int)pow((float)v1.val[2],p);
               *ma=*ma+ac;
               *mb=*mb+bc;
               *ml=*ml+lc;
...
void color_correction::gray_edge::conv2(const Mat &img, Mat& kernel, ConvolutionType type, Mat& dest) {
  Mat source = img;
  ...
  Mat tmp = kernel.clone();// todo changed by Ardillo "flip(kernel, kernel, -1)" gave an error
  flip(kernel,tmp,-1);
  kernel = tmp.clone();// todo changed by Ardillo "flip(kernel, kernel, -1)" gave an error
...
void color_correction::max_edge::conv2(const Mat &img, Mat& kernel, ConvolutionType type, Mat& dest) {
  Mat source = img;
  if(CONVOLUTION_FULL == type) {
    source = Mat();
    const int additionalRows = kernel.rows-1, additionalCols = kernel.cols-1;
